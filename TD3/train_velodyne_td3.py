...
class TD3(object):
    ...
    def train(
        ...
        if timestep % 100 == 0:
            self.writer.add_scalar("losses/qf1_values", current_Q1.mean().item(), timestep)
            self.writer.add_scalar("losses/qf2_values", current_Q2.mean().item(), timestep)
            self.writer.add_scalar("losses/qf1_loss", qf1_loss.item(), timestep)
            self.writer.add_scalar("losses/qf2_loss", qf2_loss.item(), timestep)
            self.writer.add_scalar("losses/qf_loss", qf_loss.item() / 2.0, timestep)
            self.writer.add_scalar("losses/actor_loss", actor_loss.item(), timestep)
            print("SPS:", float(timestep / (time.time() - self.start_time)))
            self.writer.add_scalar("charts/SPS", float(timestep / (time.time() - self.start_time)), timestep)
...
while timestep < max_timesteps:
    ...
    if done:
        ...
        network.writer.add_scalar("charts/episodic_return", episode_reward, timestep)
        network.writer.add_scalar("charts/episodic_length", episode_timesteps, timestep)
        episode_reward = 0
        episode_timesteps = 0
        episode_num += 1
